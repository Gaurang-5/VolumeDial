...
DDCValue convertI2CtoDDC(char *i2cBytes) {
    ...
    DDCValue invalid = {-1, -1};
    // An I2C success is not a valid VCP reply. Empty/stale/corrupt packets can
    // otherwise decode to a plausible low volume and move the dial during polling.
    if (reply[0] != 0x6e || reply[1] != 0x88 || reply[2] != 0x02 || reply[3] != 0) {
        return invalid;
    }
    UInt8 checksum = 0x50;
    ...
    uint16_t maxValue = 0;
    [[i2cData subdataWithRange:maxValueRange] getBytes:&maxValue length:sizeof(maxValue)];
    displayAttr.maxValue = CFSwapInt16BigToHost(maxValue);

    NSRange curValueRange = {8, 2};
    uint16_t curValue = 0;
    [[i2cData subdataWithRange:curValueRange] getBytes:&curValue length:sizeof(curValue)];
